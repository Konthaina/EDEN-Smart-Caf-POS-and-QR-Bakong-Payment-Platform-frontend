  ...
  <AppLayout>
    <div class="p-6 space-y-6">
      ...
      <div class="flex justify-between items-center">
        <div>
          <h1 class="text-2xl font-bold text-gray-800 dark:text-white">{{ $t('ingredient.title') }}</h1>
          <p class="text-sm text-gray-500 dark:text-gray-300">{{ $t('ingredient.subtitle') }}</p>
        </div>
        <button
          @click="openAddForm"
      ...
      <div class="text-sm text-gray-600 dark:text-gray-300">
        {{ $t('ingredient.total') }}: <strong>{{ ingredients.length }}</strong>
      </div>
      ...
      <div class="rounded-xl shadow bg-white dark:bg-gray-800 overflow-hidden">
        <div class="max-h-[480px] overflow-y-auto custom-scroll">
          <table class="min-w-full text-sm text-gray-700 dark:text-gray-200">
            <thead class="bg-purple-50 dark:bg-gray-700 text-purple-800 dark:text-purple-300 font-semibold text-left sticky top-0 z-10">
              <tr>
            ...
            <tbody>
              ...
              >
                ...
                <td class="px-4 py-3 text-center">
                  <div class="flex justify-center gap-2">
                    <button
                      class="border border-blue-300 text-blue-600 hover:bg-blue-50 dark:border-blue-500 dark:text-blue-400 dark:hover:bg-blue-800 px-3 py-1 rounded-full text-sm font-medium transition"
                      @click="editIngredient(ingredient)"
                    >
                      {{ $t('common.edit') }}
                    </button>
                    <button
                      class="border border-red-300 text-red-600 hover:bg-red-50 dark:border-red-500 dark:text-red-400 dark:hover:bg-red-800 px-3 py-1 rounded-full text-sm font-medium transition"
              ...
              <tr v-if="ingredients.length === 0">
                <td colspan="5" class="text-center py-6 text-gray-400 dark:text-gray-500">{{ $t('ingredient.noData') }}</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <teleport to="body">
        <div v-if="openModal" class="fixed inset-0 z-[999] flex items-center justify-center">
          ...
          <div class="relative z-[999] bg-white dark:bg-gray-800 w-full max-w-md rounded-xl shadow-lg p-6 space-y-4">
            ...
            <div>
              <label class="block text-sm font-medium mb-1 dark:text-gray-300">{{ $t('ingredient.name') }}</label>
              <input
                v-model="form.name"
                type="text"
            ...
            <div>
              <label class="block text-sm font-medium mb-1 dark:text-gray-300">{{ $t('ingredient.unit') }}</label>
              <input
                v-model="form.unit"
                type="text"
                class="w-full border dark:border-gray-600 rounded-lg px-3 py-2 focus:ring focus:border-purple-500 bg-white dark:bg-gray-700 dark:text-white"
      ...
      <teleport to="body">
        <div v-if="showConfirm" class="fixed inset-0 z-[999] flex items-center justify-center">
          ...
          <div class="relative z-[999] bg-white dark:bg-gray-800 p-6 rounded-xl shadow-xl space-y-4 w-full max-w-sm text-center">
            <h2 class="text-lg font-bold text-gray-800 dark:text-white">{{ $t('ingredient.confirmDeleteTitle') }}</h2>
            <p class="text-sm text-gray-600 dark:text-gray-300">{{ $t('ingredient.confirmDeleteText') }}</p>
